...
- `allowBundled`: 仅适用于 **捆绑** 技能的可选白名单。设置后，只有列表中的捆绑技能才有资格（托管/工作区技能不受影响）。
...
- `env`: 注入代理运行的环境变量（仅当未设置时）。
- `apiKey`: 对于声明了主要环境变量的技能的可选便利性。
...
当会话是 **沙盒** 时，技能进程在 Docker 内运行。沙盒 **不** 继承主机的 `process.env`。
