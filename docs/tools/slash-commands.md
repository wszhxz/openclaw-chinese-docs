...
- `commands.text` (默认 `true`) 启用在聊天消息中解析 `/...`。
  - 在没有原生命令支持的平台上（WhatsApp/WebChat/Signal/iMessage/Google Chat/MS Teams），即使将此设置为 `false`，文本命令仍然有效。
- `commands.native` (默认 `"auto"`) 注册原生命令。
  - 自动：Discord/Telegram 开启；Slack 关闭（直到你添加斜杠命令）；不支持原生命令的提供商忽略此设置。
...
- `/reasoning on|off|stream` (别名：`/reason`；开启时，发送一个以 `Reasoning:` 前缀的消息；`stream` = 仅Telegram草稿)
...
- `/reasoning`（和 `/verbose`）在群组设置中存在风险：它们可能会泄露您不打算公开的内部推理或工具输出。建议关闭它们，特别是在群聊中。
- **快速路径：** 允许列表中的发件人发送的仅命令消息会立即处理（绕过队列 + 模型）。
- **群组提及门控：** 允许列表中的发件人发送的仅命令消息会绕过提及要求。
- **内联快捷方式（仅允许列表中的发件人）：** 某些命令也可以嵌入在正常消息中，并且在模型看到剩余文本之前会被剥离。
  - 示例：`hey /status` 触发状态回复，剩余文本继续通过正常流程。
- 目前：`/help`, `/commands`, `/status`, `/whoami` (`/id`)。
- 未经授权的仅命令消息会被静默忽略，内联 `/...` 标记被视为纯文本。
- **技能命令：** `user-invocable` 技能作为斜杠命令暴露。名称被清理为 `a-z0-9_`（最多32个字符）；冲突项会附加数字后缀（例如 `_2`）。
  - `/skill <name> [input]` 通过名称运行技能（当原生命令限制阻止每个技能的命令时很有用）。
  - 默认情况下，技能命令会被转发到模型作为正常请求。
  - 技能可以选择声明 `command-dispatch: tool` 以直接将命令路由到工具（确定性，无模型）。
  - 示例：`/prose`（OpenProse 插件）— 查看 [OpenProse](/prose)。
- **原生命令参数：** Discord 使用自动完成动态选项（以及省略必需参数时的按钮菜单）。Telegram 和 Slack 在命令支持选择且省略参数时显示按钮菜单。

## 使用场景（什么显示在哪里）

- **提供者使用情况/配额**（示例：“Claude 剩余80%”）在启用使用情况跟踪时显示在当前模型提供者的 `/status` 中。
...
`/debug` 允许你设置 **仅运行时** 的配置覆盖（内存中，而非磁盘）。仅限所有者；默认情况下禁用；通过 `commands.debug: true` 启用。
