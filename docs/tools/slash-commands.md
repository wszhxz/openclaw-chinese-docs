...
仅主机使用的bash聊天命令使用 `! <cmd>`（其中 `/bash <cmd>` 是别名）。
...
- **Directives**: `/think`, `/verbose`, `/reasoning`, `/elevated`, `/exec`, `/model`, `/queue`.
  - 在模型看到消息之前，指令会被移除。
  - 在正常的聊天消息中（不是仅指令的消息），它们被视为“内联提示”，并且**不**持久化会话设置。
  - 在仅指令的消息中（消息仅包含指令），它们会持久化到会话，并回复确认信息。
  - 指令仅适用于**授权发送者**。如果设置了 `commands.allowFrom`，则它是唯一的
    允许列表；否则授权来自频道允许列表/配对加上 `commands.useAccessGroups`。
    未经授权的发送者会看到指令被视为纯文本。

还有一些**内联快捷方式**（仅允许列表/授权发送者可用）：`/help`, `/commands`, `/status`, `/whoami` (`/id`)。
...
- `commands.text` (默认 `true`) 启用在聊天消息中解析 `/...`。
  - 在不支持原生命令的界面（WhatsApp/WebChat/Signal/iMessage/Google Chat/MS Teams）上，即使设置为 `false`，文本命令仍然有效。
- `commands.native` (默认 `"auto"`) 注册原生命令。
  - 自动：Discord/Telegram 上开启；Slack 上关闭（直到你添加斜杠命令）；对于不支持原生功能的提供商忽略此设置。
...
- `commands.bashForegroundMs` (默认 `2000`) 控制 bash 在切换到后台模式之前的等待时间 (`0` 立即进入后台)。
- `commands.config` (默认 `false`) 启用 `/config` (读取/写入 `openclaw.json`)。
- `commands.debug` (默认 `false`) 启用 `/debug` (仅运行时覆盖)。
- `commands.allowFrom` (可选) 为每个提供商设置命令授权允许列表。配置后，它将成为
  命令和指令的唯一授权源（频道允许列表/配对和 `commands.useAccessGroups`
  被忽略）。使用 `"*"` 作为全局默认值；特定于提供商的密钥会覆盖它。
...
- `/export-session [path]` (别名: `/export`) (以完整系统提示导出当前会话为HTML)
- `/whoami` (显示您的发送者ID；别名: `/id`)
- `/session ttl <duration|off>` (管理会话级设置，例如TTL)
...
- `/steer <id|#> <message>` (立即引导正在运行的子代理：在运行时可能进行，在运行时无法则中止当前工作并根据引导消息重新启动)
...
- `/tts off|always|inbound|tagged|status|provider|limit|summary|audio` (控制TTS；参见[/tts](/tts))
  - Discord: 原生命令是 `/voice` (Discord保留 `/tts`)；文本 `/tts` 仍然有效。
- `/stop`
- `/restart`
...
- `/reasoning on|off|stream` (别名: `/reason`；启用时，发送带有前缀 `Reasoning:` 的单独消息；`stream` = 仅Telegram草稿)
...
- `! <command>` (仅限主机；一次一个；对于长时间运行的任务使用 `!poll` + `!stop`)
- `!poll` (检查输出/状态；接受可选的 `sessionId`；`/bash poll` 也可以使用)
- `!stop` (停止正在运行的bash任务；接受可选的 `sessionId`；`/bash stop` 也可以使用)

注释:

- 命令接受一个可选的 `:` 在命令和参数之间（例如 `/think: high`, `/send: on`, `/help:`）。
...
- 绑定到 Discord 线程的命令 (`/focus`, `/unfocus`, `/agents`, `/session ttl`) 需要启用有效的线程绑定 (`session.threadBindings.enabled` 和/或 `channels.discord.threadBindings.enabled`)。
- `/verbose` 用于调试和额外的可见性；在正常使用中应保持 **关闭**。
- `/reasoning` (和 `/verbose`) 在群组设置中存在风险：它们可能会泄露您不打算公开的内部推理或工具输出。最好关闭它们，尤其是在群聊中。
- **快速路径：** 允许列表中的发件人发送的仅命令消息会立即处理（绕过队列 + 模型）。
- **群组提及门控：** 允许列表中的发件人发送的仅命令消息会绕过提及要求。
- **内联快捷方式（仅允许列表中的发件人）：** 某些命令也可以嵌入在正常消息中，并在模型看到剩余文本之前被剥离。
  - 示例：`hey /status` 触发状态回复，而剩余文本继续通过正常流程。
- 目前：`/help`, `/commands`, `/status`, `/whoami` (`/id`)。
- 未经授权的仅命令消息会被静默忽略，内联 `/...` 标记被视为纯文本。
- **技能命令：** `user-invocable` 技能作为斜杠命令暴露。名称被清理为 `a-z0-9_`（最大 32 个字符）；冲突会附加数字后缀（例如 `_2`）。
  - `/skill <name> [input]` 按名称运行技能（当原生命令限制阻止按技能的命令时很有用）。
  - 默认情况下，技能命令作为正常请求转发给模型。
  - 技能可以选择声明 `command-dispatch: tool` 将命令直接路由到工具（确定性，无模型）。
  - 示例：`/prose` (OpenProse 插件) — 请参阅 [OpenProse](/prose)。
- **原生命令参数：** Discord 使用自动完成功能动态选项（以及省略必需参数时的按钮菜单）。Telegram 和 Slack 在命令支持选择且省略参数时显示按钮菜单。

## 使用场景（什么在哪里显示）

- **Provider usage/quota** (示例: “Claude 80% left”) 在 `/status` 中显示当前模型提供商的使用情况，当启用使用跟踪时。
- **Per-response tokens/cost** 由 `/usage off|tokens|full` 控制（附加到正常回复中）。
- `/model status` 关于 **models/auth/endpoints**，而不是使用情况。
...
- 在 Discord 上，`/model` 和 `/models` 打开一个交互式选择器，带有提供商和模型下拉菜单以及一个提交步骤。
- `/model <#>` 从该选择器中选择（如果可能，优先选择当前提供商）。
- `/model status` 显示详细视图，包括配置的提供商端点 (`baseUrl`) 和 API 模式 (`api`)（如果有）。

## 调试覆盖

`/debug` 允许你设置 **运行时** 配置覆盖（内存中，不是磁盘）。仅限所有者；默认情况下禁用；通过 `commands.debug: true` 启用。
...
`/config` 写入你的磁盘配置 (`openclaw.json`)。仅限所有者；默认情况下禁用；通过 `commands.config: true` 启用。
...
- **Slack:** `channels.slack.slashCommand` 仍然支持单个 `/openclaw` 样式的命令。如果你启用 `commands.native`，必须为每个内置命令创建一个 Slack 斜杠命令（与 `/help` 名称相同）。Slack 的命令参数菜单以临时 Block Kit 按钮的形式提供。