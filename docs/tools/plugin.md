...
Npm规范仅限于**注册表**（包名称+可选版本/标签）。Git/URL/文件规范会被拒绝。

3. 重启网关，然后在`plugins.entries.<id>.config`下进行配置。

查看[语音通话](/plugins/voice-call)以获取具体示例插件。
寻找第三方列表？请参阅[社区插件](/plugins/community)。

## 可用插件（官方）

- 自2026.1.15起，Microsoft Teams 仅作为插件提供；如果你使用 Teams，请安装 `@openclaw/msteams`。
...
- Copilot Proxy（提供商认证）——本地 VS Code Copilot Proxy 桥接；与内置的 `github-copilot` 设备登录不同（捆绑，默认禁用）

OpenClaw 插件是通过 jiti 在运行时加载的**TypeScript 模块**。**配置验证不执行插件代码**；它使用插件清单和 JSON Schema 而不是插件代码。查看[插件清单](/plugins/manifest)。
...
- 返回 PCM 音频缓冲区 + 采样率。插件必须重新采样/编码以供提供商使用。
- Edge TTS 不支持电话。
...
- 如果 `plugins.allow` 为空且可发现非内置插件，OpenClaw 在启动时会记录包含插件 ID 和来源的警告。
- 候选路径在发现之前会进行安全检查。当以下情况发生时，OpenClaw 会阻止候选路径：
  - 扩展条目解析到插件根目录之外（包括符号链接/路径遍历逃逸），
  - 插件根目录/源路径对所有人可写，
  - 对于非内置插件，路径所有权可疑（POSIX 所有者既不是当前用户 ID 也不是 root）。
...
如果有多个插件解析为相同的 ID，则上述顺序中的第一个匹配项获胜，较低优先级的副本将被忽略。
...
安全防护：每个 `openclaw.extensions` 条目在符号链接解析后必须保持在插件
目录内。逃逸包目录的条目将被拒绝。

安全说明：`openclaw plugins install` 使用 `npm install --ignore-scripts` 安装插件依赖项（不运行生命周期脚本）。保持插件依赖树为“纯 JS/TS”，避免需要 `postinstall` 构建的包。

### 通道目录元数据

通道插件可以通过 `openclaw.channel` 广告入门元数据并通过 `openclaw.install` 提供安装提示。这使核心目录数据保持无污染。
...
OpenClaw 还可以合并 **external channel catalogs**（例如，一个 MPM 注册表导出）。将一个 JSON 文件拖放到以下位置之一：
...
如果插件导出了 `id`，OpenClaw 使用它，但如果与配置的 ID 不匹配则会发出警告。
...
- 除非插件清单声明了通道 ID，否则未知的 `channels.<id>` 键是 **错误**。
- 使用嵌入在 `openclaw.plugin.json` (`configSchema`) 中的 JSON Schema 验证插件配置。
- 如果禁用了插件，则保留其配置并发出 **警告**。

## 插件槽（互斥类别）

某些插件类别是 **互斥** 的（一次只有一个活动）。使用 `plugins.slots` 选择哪个插件拥有该插槽：
...
如果有多个插件声明了 `kind: "memory"`，则只有选定的一个会被加载。其他插件会被禁用并进行诊断。
...
如果存储的完整性元数据在更新之间发生变化，OpenClaw 会发出警告并请求确认（使用全局 `--yes` 跳过提示）。
...
插件可以打包钩子并在运行时注册它们。这允许插件捆绑事件驱动的自动化，而无需单独安装钩子包。
...
- 由插件管理的钩子会显示在 `openclaw hooks list` 中带有 `plugin:<id>`。
- 您不能通过 `openclaw hooks` 启用或禁用由插件管理的钩子；请启用或禁用插件。

## 提供程序插件（模型认证）

插件可以注册 **模型提供程序认证** 流程，以便用户可以在 OpenClaw 内部运行 OAuth 或 API 密钥设置（无需外部脚本）。

通过 `api.registerProvider(...)` 注册一个提供程序。每个提供程序公开一种或多种认证方法（OAuth、API 密钥、设备代码等）。这些方法支持：
...
- `meta.preferOver` 列出在两个都配置时要跳过自动启用的通道 ID。
- `meta.detailLabel` 和 `meta.systemImage` 允许 UI 显示更丰富的通道标签/图标。

### 编写新的消息通道（分步指南）

当您想要一个新的**聊天界面**（一个“消息通道”），而不是模型提供者时使用此方法。模型提供者文档位于 `/providers/*` 下。
...
- `meta.preferOver` 允许插件替换另一个通道（自动启用时优先选择它）。
...
插件可以注册自定义斜杠命令，这些命令**不会调用 AI 代理**执行。这对于开关命令、状态检查或不需要 LLM 处理的快速操作非常有用。
...
- `acceptsArgs`: 命令是否接受参数（默认：false）。如果为false且提供了参数，命令将不会匹配，并且消息将传递给其他处理程序
...
插件与网关在同一个进程中运行。将它们视为受信任的代码：

- 仅安装您信任的插件。
- 倾向于使用 `plugins.allow` 允许列表。
- 在更改后重启网关。
