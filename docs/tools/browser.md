...
- `chrome`：通过扩展中继到您的 **系统浏览器**（需要将 OpenClaw 扩展附加到一个标签页）。

如果希望默认使用受管模式，请设置 `browser.defaultProfile: "openclaw"`。
...
- 浏览器控制服务绑定到一个从 `gateway.port` 派生的回环端口（默认：`18791`，即网关 + 2）。中继使用下一个端口(`18792`)。
- 如果您覆盖了网关端口(`gateway.port` 或 `OPENCLAW_GATEWAY_PORT`)，
  派生的浏览器端口会相应地移动以保持在同一“系列”中。
- `cdpUrl` 在未设置时默认为中继端口。
- `remoteCdpTimeoutMs` 适用于远程（非回环）CDP可达性检查。
...
如果您在具有浏览器的机器上运行**节点主机**，OpenClaw可以
...
[Browserless](https://browserless.io) 是一个托管的 Chromium 服务，通过 HTTPS 暴露 CDP 端点。您可以将 OpenClaw 浏览器配置文件指向一个 Browserless 区域端点，并使用您的 API 密钥进行身份验证。
...
- 将网关和任何节点主机保留在私有网络（如 Tailscale）中；避免公开暴露。
...
- **remote**：一个显式的 CDP URL（运行在其他地方的基于 Chromium 的浏览器）
- **extension relay**：通过本地中继 + Chrome 扩展程序使用您现有的 Chrome 标签页

默认设置：

- 如果缺少，会自动创建 `openclaw` 配置文件。
- `chrome` 配置文件内置用于 Chrome 扩展程序中继（默认指向 `http://127.0.0.1:18792`）。
...
OpenClaw 还可以通过本地 CDP 中继 + Chrome 扩展程序驱动 **您现有的 Chrome 标签页**（无需单独的“openclaw” Chrome 实例）。

完整指南：[Chrome 扩展程序](/tools/chrome-extension)

流程：

- 网关在本地运行（同一台机器）或节点主机在浏览器机器上运行。
- 本地 **中继服务器** 监听回环 `cdpUrl`（默认：`http://127.0.0.1:18792`）。
- 您点击标签页上的 **OpenClaw Browser Relay** 扩展程序图标以附加（它不会自动附加）。
- 代理通过正常 `browser` 工具控制该标签页，通过选择正确的配置文件。
...
- 钉住扩展程序，然后点击你想要控制的标签页（徽章显示 `ON`）。

2. 使用方法：

- 命令行界面：`openclaw browser --browser-profile chrome tabs`
- 代理工具：使用 `browser` 和 `profile="chrome"`

可选：如果你想使用不同的名称或中继端口，请创建自己的配置文件：
...
- 此模式依赖于 Playwright-on-CDP 进行大多数操作（截图/快照/动作）。
- 通过再次点击扩展图标来分离。
...
你可以使用 `browser.executablePath` 来覆盖。
...
一些功能（navigate/act/AI snapshot/role snapshot, element screenshots, PDF）需要
...
`OPENCLAW_HOME_VOLUME` 或绑定挂载进行持久化。参见 [Docker](/install/docker)。
...
- `snapshot`:
  ...
  - `--interactive` 输出一个易于选择的交互元素的平面列表（最适合驱动操作）。
...
- **角色快照（角色引用如 `e12`）**：`openclaw browser snapshot --interactive`（或 `--compact`, `--depth`, `--selector`, `--frame`）
  ...
  - 内部，通过 `getByRole(...)` 解析引用（对于重复项加上 `nth()`）。
  - 添加 `--labels` 以包括带有叠加 `e12` 标签的视口截图。

引用行为：

- 引用 **在导航之间不稳定**；如果出现问题，请重新运行 `snapshot` 并使用新的引用。
- 如果角色快照是通过 `--frame` 获取的，则角色引用被限定在该 iframe 内，直到下一个角色快照。
...
JSON 中的角色快照包括 `refs` 以及一个小的 `stats` 块（行/字符/引用/交互），以便工具可以推理有效负载的大小和密度。

## 状态和环境设置

这些对于“使站点表现得像 X”工作流程很有用：
...
- `browser act kind=evaluate` / `openclaw browser evaluate` 和 `wait --fn`
  ...
  这个过程。如果不需要，使用 `browser.evaluateEnabled=false` 禁用它。
- 对于登录和反机器人说明（X/Twitter 等），请参阅 [浏览器登录 + X/Twitter 发帖](/tools/browser-login)。
- 保持网关/节点主机私密（回环或仅限 Tailnet）。
- 远程 CDP 终端非常强大；请对其进行隧道传输和保护。

## 故障排除

对于特定于 Linux 的问题（尤其是 snap Chromium），请参阅
...
- `browser` — 状态/启动/停止/打开标签页/聚焦/关闭/快照/截图/导航/操作
...
- `browser` 接受：
  ...
  - 如果连接了支持浏览器的节点，工具可能会自动路由到该节点，除非你固定了 `target="host"` 或 `target="node"`。
