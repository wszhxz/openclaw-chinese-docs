...
- **审批**：`full` 跳过执行审批；`on`/`ask` 在白名单/询问规则要求时认可它们。
...
- 如果提升访问被禁用或发送者不在批准的白名单上，指令会回复一个可操作的错误并且不会更改会话状态。
...
- 每个代理的白名单：`agents.list[].tools.elevated.allowFrom`（可选；设置时，发送者必须匹配 **全局** + **每个代理** 的白名单）。
- Discord 回退：如果省略 `tools.elevated.allowFrom.discord`，则使用 `channels.discord.dm.allowFrom` 列表作为回退。设置 `tools.elevated.allowFrom.discord`（甚至 `[]`）以覆盖。每个代理的白名单 **不** 使用回退。
- 所有门控必须通过；否则提升被视为不可用。
