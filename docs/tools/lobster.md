...
龙虾是一个工作流外壳，允许OpenClaw以单个、确定性操作的形式运行多步骤工具序列，并具有显式的批准检查点。

## Hook

您的助手可以构建管理自身的工具。请求一个工作流，30分钟后您将获得一个CLI加上作为一次调用运行的管道。龙虾是缺失的部分：确定性的管道、显式的批准和可恢复的状态。

## 为什么

今天，复杂的流程需要许多来回的工具调用。每次调用都会消耗令牌，并且LLM必须协调每一步骤。龙虾将这种协调移动到一个类型化的运行时中：

- **一次调用而不是多次**：OpenClaw运行一次龙虾工具调用并获得一个结构化结果。
- **内置批准**：副作用（发送邮件、发布评论）会暂停工作流，直到显式批准。
- **可恢复**：暂停的工作流会返回一个令牌；批准后无需重新运行所有步骤即可恢复。
...
- **确定性+可审计性**：管道是数据，因此易于记录、差异比较、重播和审查。
- **AI受限表面**：小语法+JSON管道减少了“创造性”的代码路径，并使验证变得现实。
- **内置安全策略**：超时、输出上限、沙箱检查和白名单由运行时强制执行，而不是每个脚本。
- **仍然可编程**：每个步骤可以调用任何CLI或脚本。如果您想要JS/TS，可以从代码生成`.lobster`文件。

## 它是如何工作的

OpenClaw以**工具模式**启动本地`lobster` CLI并解析stdout中的JSON信封。
如果管道暂停以等待批准，工具会返回一个`resumeToken`，以便稍后继续。

## 模式：小型CLI + JSON管道 + 批准

构建小型命令以使用JSON通信，然后将它们链接成单个龙虾调用。（下面示例命令名称——替换为您自己的。）
...
AI触发工作流；龙虾执行步骤。批准门将副作用保持显式和可审计。
...
参见[LLM任务](/tools/llm-task)获取详细信息和配置选项。
...
在与OpenClaw网关运行在同一主机上安装龙虾CLI（参见[Lobster仓库](https://github.com/openclaw/lobster)），并确保`lobster`位于`PATH`。
...
注意：可选插件的白名单是可选的。如果白名单仅命名插件工具（如`lobster`），OpenClaw将保持核心工具启用。要限制核心工具，请在白名单中包含您想要的核心工具或组。
...
- `cwd`：管道的相对工作目录（必须保持在当前进程的工作目录内）。
- `timeoutMs`：如果子进程超出此持续时间则终止（默认：20000）。
- `maxStdoutBytes`：如果stdout超出此大小则终止子进程（默认：512000）。
...
OpenProse与龙虾配合良好：使用`/prose`编排多代理准备，然后运行龙虾管道以进行确定性批准。如果Prose程序需要龙虾，请通过`tools.subagents.tools`为子代理允许`lobster`工具。参见[OpenProse](/prose)。

## 安全

- **仅本地子进程** — 插件本身不会进行网络调用。
- **无秘密** — 龙虾不管理OAuth；它调用OpenClaw工具来处理。
- **沙盒感知** — 当工具上下文被沙盒化时禁用。
- **加固** — 固定的可执行文件名(`lobster`)在`PATH`；强制执行超时和输出上限。
...
一个公共示例：一个“第二大脑”CLI + 龙虾管道，管理三个Markdown保险库（个人、合作伙伴、共享）。CLI发出用于统计、收件箱列表和过时扫描的JSON；龙虾将这些命令链接成工作流，如`weekly-review`、`inbox-triage`、`memory-consolidation`和`shared-task-sync`，每个工作流都有批准门。AI在可用时处理判断（分类），并在不可用时回退到确定性规则。
