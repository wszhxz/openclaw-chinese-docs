...
子代理是从现有代理运行中生成的后台代理运行。它们在自己的会话 (`agent:<agentId>:subagent:<uuid>`) 中运行，并且在完成后，**通告**其结果回请求者的聊天频道。
...
成本说明：每个子代理都有自己的**上下文**和**令牌使用量**。对于繁重或重复的任务，为子代理设置更便宜的模型，并将主代理保持在更高质量的模型上。您可以通过 `agents.defaults.subagents.model` 或每个代理的覆盖来配置此设置。
...
- 默认模型：继承调用者除非设置了 `agents.defaults.subagents.model`（或每个代理的 `agents.list[].subagents.model`）；显式的 `sessions_spawn.model` 仍然优先。
- 默认思考：继承调用者除非设置了 `agents.defaults.subagents.thinking`（或每个代理的 `agents.list[].subagents.thinking`）；显式的 `sessions_spawn.thinking` 仍然优先。
...
- `model?`（可选；覆盖子代理模型；无效值会被跳过，并且子代理会在默认模型上运行并在工具结果中发出警告）
...
- 否则，通告回复通过后续的 `agent` 调用 (`deliver=true`) 发布到请求者的聊天频道。
...
- 在请求者的聊天中发送 `/stop` 终止请求者会话并停止由此生成的任何活动子代理运行。

## 限制

- 子代理通告是**尽力而为**的。如果网关重启，待处理的“通告回”工作将丢失。
- 子代理仍然共享相同的网关进程资源；将 `maxConcurrent` 视为安全阀。
- `sessions_spawn` 总是非阻塞的：它会立即返回 `{ status: "accepted", runId, childSessionKey }`。
