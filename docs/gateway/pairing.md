...
在网关拥有配对中，**网关** 是决定哪些节点可以加入的权威来源。UI（macOS 应用程序、未来的客户端）只是前端，用于批准或拒绝待处理请求。

**重要：** WS 节点在 `connect` 期间使用 **设备配对**（角色 `node`）。
`node.pair.*` 是一个独立的配对存储，并且 **不** 作为 WS 握手的门控。
...
- **传输**：网关 WS 终端转发请求但不决定成员资格。（遗留的 TCP 桥接支持已弃用/移除。）
...
3. 您批准或拒绝请求（CLI 或 UI）。
4. 在批准后，网关颁发一个新的 **令牌**（重新配对时会轮换令牌）。
5. 节点使用该令牌重新连接，现在“已配对”。
...
macOS 应用程序可以在以下情况下尝试 **静默批准**：

- 请求被标记为 `silent`，并且
- 应用程序可以使用同一用户验证与网关主机的 SSH 连接。
...
如果您覆盖 `OPENCLAW_STATE_DIR`，则 `nodes/` 文件夹会随之移动。
