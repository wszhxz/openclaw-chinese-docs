...
OpenClaw 可以在 **Docker 容器内部运行工具** 以减少影响范围。
这是 **可选** 的，并由配置控制 (`agents.defaults.sandbox` 或
`agents.list[].sandbox`)。如果禁用了沙箱，工具将在主机上运行。
网关保留在主机上；启用时，工具执行将在隔离的沙箱中运行。
...
- 可选的沙箱浏览器 (`agents.defaults.sandbox.browser`)。
  ...
  - noVNC 观察者访问默认是密码保护的；OpenClaw 发射一个短期令牌 URL，解析为观察者会话。
  - `agents.defaults.sandbox.browser.allowHostControl` 允许沙箱化会话显式地针对主机浏览器。
...
- 任何明确允许在主机上运行的工具（例如 `tools.elevated`）。
  ...
  - 如果禁用了沙箱，`tools.elevated` 不会更改执行（已经在主机上）。参见 [提升模式](/tools/elevated)。
...
- `"off"`: 不使用沙箱。
- `"non-main"`: 仅对 **非主** 会话使用沙箱（如果您希望正常聊天在主机上，默认设置）。
- `"all"`: 每个会话都在沙箱中运行。
...
技能注意事项：`read` 工具是基于沙箱根目录的。使用 `workspaceAccess: "none"`，
...
`agents.defaults.sandbox.browser.binds` 仅将附加的主机目录挂载到 **沙箱浏览器** 容器中。

- 当设置时（包括 `[]`），它将覆盖浏览器容器中的 `agents.defaults.sandbox.docker.binds`。
...
- 敏感挂载（机密信息、SSH 密钥、服务凭证）除非绝对必要，否则应 `:ro`。
- 结合 `workspaceAccess: "ro"` 如果您只需要对工作区的读访问；绑定模式保持独立。
- 参见 [沙箱 vs 工具策略 vs 提升](/gateway/sandbox-vs-tool-policy-vs-elevated) 了解绑定如何与工具策略和提升执行交互。
...
- 默认的 `docker.network` 是 `"none"`（无出口），因此包安装将会失败。
- `readOnlyRoot: true` 防止写入；设置 `readOnlyRoot: false` 或制作自定义镜像。
- `user` 必须是 root 才能进行包安装（省略 `user` 或设置 `user: "0:0"`)。
- 沙箱 exec **不** 继承主机的 `process.env`。使用
  `agents.defaults.sandbox.docker.env`（或自定义镜像）用于技能 API 密钥。

## 工具策略 + 逃生舱

工具允许/拒绝策略仍然适用于沙箱规则之前。如果工具被全局或每个代理拒绝，沙箱不会将其恢复。

`tools.elevated` 是一个显式的逃生舱，在主机上运行 `exec`。
`/exec` 指令仅适用于授权发送者，并且每个会话持续有效；要硬禁用
`exec`，使用工具策略拒绝（参见 [沙箱 vs 工具策略 vs 提升](/gateway/sandbox-vs-tool-policy-vs-elevated)）。

调试：

- 使用 `openclaw sandbox explain` 检查有效的沙箱模式、工具策略和修复配置键。
- 参见 [沙箱 vs 工具策略 vs 提升](/gateway/sandbox-vs-tool-policy-vs-elevated) 了解“为什么这被阻止？”的心理模型。
  保持锁定状态。
...
参见 [多代理沙箱 & 工具](/tools/multi-agent-sandbox-tools) 了解优先级。
...
- [多代理沙箱 & 工具](/tools/multi-agent-sandbox-tools)
