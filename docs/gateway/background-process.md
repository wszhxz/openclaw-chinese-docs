...
当在 exec/process 工具之外生成长时间运行的子进程（例如，CLI 重启或网关辅助程序），附加子进程桥接辅助程序以便转发终止信号并在退出/错误时分离监听器。这可以避免 systemd 上的孤立进程，并使不同平台上的关闭行为一致。
...
- `PI_BASH_JOB_TTL_MS`: 完成会话的 TTL (毫秒, 限制在 1 分钟到 3 小时之间)
