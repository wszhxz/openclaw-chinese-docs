...
**声明：** 配对存储应在交错情况下强制执行`MaxPending` 和幂等性（即，“检查-然后写入”必须是原子的/锁定的；刷新不应创建重复项）。

这意味着：

- 在并发请求下，您不能超过`MaxPending` 对于一个频道。
...
**声明：** 摄入应跨扇出保留跟踪关联，并在提供者重试下保持幂等性。

这意味着：

- 当一个外部事件变成多个内部消息时，每个部分保持相同的跟踪/事件标识。
- 重试不会导致双重处理。
- 如果提供者事件ID丢失，去重将回退到一个安全键（例如，跟踪ID）以避免丢弃不同的事件。
...
**声明：** 路由必须默认隔离DM会话，并且仅在显式配置时合并会话（频道优先级 + 身份链接）。

这意味着：

- 特定于频道的dmScope覆盖必须优于全局默认值。
