...
如果您希望节点显示代理可以在磁盘上编辑的实际 HTML/CSS/JS，请将节点指向网关画布主机。

注意：节点从网关 HTTP 服务器加载画布（与 `gateway.port` 相同端口，默认 `18789`）。
...
尾网（可选）：如果两个设备都在 Tailscale 上，使用 MagicDNS 名称或尾网 IP 而不是 `.local`，例如 `http://<gateway-magicdns>:18789/__openclaw__/canvas/`。
