...
如果您尝试允许 **语音识别** 或 **麦克风** 访问时应用程序崩溃，可能是由于 TCC 缓存损坏或签名不匹配。
...
1. 重置 TCC 权限：
   ...
   ```
2. 如果上述方法无效，请临时更改 [`scripts/package-mac-app.sh`](https://github.com/openclaw/openclaw/blob/main/scripts/package-mac-app.sh) 中的 `BUNDLE_ID` 以强制 macOS 进入“干净状态”。
