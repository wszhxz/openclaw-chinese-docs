...
---
# macOS上的网关 (外部 launchd 启动)

OpenClaw.app 不再捆绑 Node/Bun 或网关运行时。macOS 应用程序
期望一个 **外部** `openclaw` CLI 安装，不作为子进程启动网关，并管理每个用户的 launchd 服务以保持网关运行（或者如果已经有一个本地网关在运行，则附加到该网关）。
...
macOS 应用程序的 **Install CLI** 按钮通过 npm/pnpm 运行相同的流程（不推荐使用 bun 作为网关运行时）。
