...
受众：macOS 应用贡献者。目标：在唤醒词和按住说话重叠时保持语音叠加的可预测性。

### 当前意图

- 如果叠加已经因为唤醒词而可见，并且用户按下热键，热键会话会_采用_现有的文本而不是重置它。叠加在热键被按住时保持显示。当用户释放热键时：如果有修剪后的文本则发送，否则dismiss。
- 单独的唤醒词仍然会在静默时自动发送；按住说话在释放时立即发送。

### 已实现 (2025年12月9日)

- 叠加会话现在每个捕获（唤醒词或按住说话）携带一个令牌。当令牌不匹配时，丢弃部分/最终/发送/dismiss/级别更新，避免过时的回调。
- 按住说话会采用任何可见的叠加文本作为前缀（因此在唤醒叠加显示时按下热键会保留文本并附加新的语音）。它最多等待1.5秒以获取最终的转录，否则回退到当前文本。
- 铃声/叠加日志在`info`中以类别`voicewake.overlay`，`voicewake.ptt`，和`voicewake.chime`（会话开始，部分，最终，发送，dismiss，铃声原因）发出。
...
4. **统一发送路径**
   - 在`endCapture`：如果修剪后的文本为空 → dismiss；否则 `performSend(session:)`（播放一次发送铃声，转发，dismiss）。
   - 按住说话：无延迟；唤醒词：可选延迟用于自动发送。
   - 在按住说话结束后对唤醒运行时应用短暂的冷却，以防止唤醒词立即重新触发。
5. **日志记录**
...
- 确保按住说话释放总是使用活动令牌调用`endCapture`；如果文本为空，预期`dismiss`没有铃声或发送。
