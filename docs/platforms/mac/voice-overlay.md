...
- 如果叠加已经因为唤醒词而可见，并且用户按下热键，热键会话会_采用_现有文本而不是重置它。叠加在热键被按住时保持显示。当用户释放热键时：如果有修剪后的文本则发送，否则关闭。
- 单独的唤醒词仍然会在静默时自动发送；按住说话在释放时立即发送。

### 已实现 (2025年12月9日)

- 叠加会话现在每个捕获（唤醒词或按住说话）携带一个令牌。当令牌不匹配时，丢弃部分/最终/发送/关闭/级别更新，避免过时的回调。
...
3. **叠加绑定**
   ...
   - 叠加用户操作 (`sendNow`，`dismiss`，`edit`) 回调到协调器并附带会话令牌。
4. **统一发送路径**
   - 在`endCapture`：如果修剪后的文本为空 → 关闭；否则 `performSend(session:)`（播放发送铃声一次，转发，关闭）。
   - 按住说话：无延迟；唤醒词：自动发送可选延迟。
   - 在按住说话结束后对唤醒运行时应用短暂冷却，以防止唤醒词立即重新触发。
5. **日志记录**
...
- 确保按住说话释放总是使用活动令牌调用 `endCapture`；如果文本为空，预期 `dismiss` 无铃声或发送。
