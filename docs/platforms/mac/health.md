...
- 通用选项卡新增一个健康卡片，显示：已链接认证时间、会话存储路径/数量、上次检查时间、上次错误/状态码，以及运行健康检查/显示日志的按钮。
- 使用缓存的快照，使UI即时加载，并在网络离线时优雅降级。
- **通道选项卡**显示通道状态以及WhatsApp/Telegram（登录二维码、登出、探测、上次断开/错误）的控制选项。

## 探测的工作原理

- 应用程序每约60秒通过`ShellExecutor`运行一次`openclaw health --json`，并按需运行。探测加载凭据并报告状态而不发送消息。
