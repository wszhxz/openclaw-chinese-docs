...
macOS 应用程序使用 `WKWebView` 嵌入一个由代理控制的 **Canvas 面板**。它是一个轻量级的可视化工作区，适用于 HTML/CSS/JS、A2UI 和小型交互式 UI 表面。
...
可以通过设置 → **允许 Canvas** 禁用 Canvas。当禁用时，canvas 节点命令返回 `CANVAS_DISABLED`。
...
A2UI 由网关 canvas 主机托管，并在 Canvas 面板中渲染。当网关通告一个 Canvas 主机时，macOS 应用程序在首次打开时自动导航到 A2UI 主机页面。
