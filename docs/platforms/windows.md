...
WSL有自己的虚拟网络。如果另一台机器需要访问运行在**WSL内部**的服务（SSH，本地TTS服务器，或Gateway），必须将Windows端口转发到当前WSL IP。WSL IP在重启后会更改，因此您可能需要刷新转发规则。
...
- 从另一台机器进行SSH连接时，目标是**Windows主机IP**（例如：`ssh user@windows-host -p 2222`）。
- 远程节点必须指向一个**可访问**的Gateway URL（不是`127.0.0.1`）；使用
...
我们目前还没有Windows配套应用程序。如果您愿意贡献，欢迎参与开发。