...
macOS 应用程序是 OpenClaw 的 **菜单栏伴侣**。它拥有权限，管理/连接到本地网关（通过 launchd 或手动），并向代理暴露 macOS 功能作为节点。
...
应用程序管理一个每个用户的 LaunchAgent，标记为 `bot.molt.gateway`
...
- 当无头节点主机服务正在运行（远程模式）时，它会作为节点连接到网关 WS。
- `system.run` 在 macOS 应用程序（UI/TCC 上下文）中通过本地 Unix 套接字执行；提示和输出保留在应用内。
...
`system.run` 由 macOS 应用程序中的 **执行审批** 控制（设置 → 执行审批）。
安全性和白名单存储在 Mac 的本地位置：
...
使用调试 CLI 来练习与 macOS 应用程序相同的网关 WebSocket 握手和发现逻辑，而无需启动应用程序。
...
- **IP 报告**：SSH 隧道使用回环，因此网关会看到节点 IP 为 `127.0.0.1`。如果您希望真实的客户端 IP 出现，请使用 **直接 (ws/wss)** 传输（参见 [macOS 远程访问](/platforms/mac/remote)）。
