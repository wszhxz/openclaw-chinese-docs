...
macOS 应用程序是 OpenClaw 的 **菜单栏伴侣**。它拥有权限，管理/连接到本地网关（launchd 或手动），并向代理暴露 macOS 功能作为节点。
...
当运行命名配置文件时，将标签替换为 `bot.molt.<profile>`。
...
- **远程端口**：远程主机上的相同网关端口。
- **行为**：没有随机本地端口；应用程序重用现有的健康隧道或在需要时重新启动它。
- **SSH 形状**：`ssh -N -L <local>:127.0.0.1:<remote>` 带有 BatchMode +
  ExitOnForwardFailure + 保持活动选项。
- **IP 报告**：SSH 隧道使用回环，因此网关会看到节点 IP 为 `127.0.0.1`。如果您希望实际客户端 IP 出现，请使用 **直接 (ws/wss)** 传输（参见 [macOS 远程访问](/platforms/mac/remote)）。

设置步骤，请参阅 [macOS 远程访问](/platforms/mac/remote)。协议详细信息，请参阅 [网关协议](/gateway/protocol)。
