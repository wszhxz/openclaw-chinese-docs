...
有关CoreDNS示例，请参阅[Bonjour](/gateway/bonjour)。
...
- 它由网关HTTP服务器提供服务（与`gateway.port`相同的端口，默认`18789`）。
- 当广告了画布主机URL时，iOS节点在连接时会自动导航到A2UI。
...
- 语音唤醒和对话模式在设置中可用。
- 当应用程序不活动时，iOS可能会挂起后台音频；将语音功能视为尽力而为。

## 常见错误

- `NODE_BACKGROUND_UNAVAILABLE`：将iOS应用带到前台（画布/摄像头/屏幕命令需要它）。
