...
本指南将 OpenClaw 网关从一台机器迁移到另一台 **无需重新进行初始设置**。

迁移的概念很简单：

- 复制 **状态目录** (`$OPENCLAW_STATE_DIR`，默认：`~/.openclaw/`) — 这包括配置、认证、会话和通道状态。
- 复制你的 **工作区** (`~/.openclaw/workspace/` 默认) — 这包括你的代理文件（内存、提示词等）。
...
查找输出中的 `OPENCLAW_STATE_DIR` / 配置文件的提及。如果你运行多个网关，请对每个配置文件重复此操作。
...
你的工作区是存储 `MEMORY.md`、`USER.md` 和 `memory/*.md` 等文件的地方。
...
此时，如果初始设置创建了一个新的 `~/.openclaw/` 是可以的 — 你将在下一步覆盖它。
...
如果你使用配置文件（或 `OPENCLAW_STATE_DIR`）运行了旧网关，而新网关使用不同的配置文件，你会看到如下症状：
...
仅复制 `openclaw.json` 是不够的。许多提供商将状态存储在：
...
- [OpenClaw 存储其数据的位置？](/help/faq#where-does-openclaw-store-its-data)