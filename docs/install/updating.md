...
**首选**的更新路径是从网站重新运行安装程序。它会检测现有安装，进行就地升级，并在需要时运行 `openclaw doctor`。
...
参见 [开发渠道](/install/development-channels) 了解渠道语义和发行说明。
...
如果您是通过 **npm/pnpm** 安装（没有 git 元数据），`openclaw update` 将尝试通过包管理器进行更新。如果无法检测到安装，请改用“更新（全局安装）”。

## 更新（控制界面 / RPC）

控制界面具有 **更新并重启**（RPC: `update.run`)。它会：
...
- 在全局和 git 安装之间切换很容易：安装另一种类型，然后运行 `openclaw doctor` 以便网关服务入口点重写为当前安装。

## 总是运行：`openclaw doctor`

Doctor 是“安全更新”命令。它故意设计得非常简单：修复 + 迁移 + 警告。

注意：如果您是**源码安装**（git checkout），`openclaw doctor` 将提供先运行 `openclaw update` 的选项。

它通常会做以下事情：

- 迁移已弃用的配置键/旧版配置文件位置。
- 审核 DM 策略并在发现风险较高的“开放”设置时发出警告。
- 检查网关健康状况并可以提供重启选项。
- 检测并迁移较旧的网关服务（launchd/systemd；旧版 schtasks）到当前的 OpenClaw 服务。
- 在 Linux 上，确保 systemd 用户持久性（以便网关在注销后仍能运行）。
...
选择一个特定日期的提交（示例：“2026-01-01 的主分支状态”）：
