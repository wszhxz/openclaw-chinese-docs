...
注意：**Model** 部分现在包括一个用于选择 `agents.defaults.models` 允许列表的多选框（显示在 `/model` 和模型选择器中）。
...
- 基于频道的服务（Slack/Discord/Matrix/Microsoft Teams）在设置期间会提示输入频道/房间允许列表。可以输入名称或ID；向导会在可能的情况下将名称解析为ID。
