...
使用 `--connected` 仅显示当前连接的节点。使用 `--last-connected <duration>` 过滤在指定时间段内连接的节点（例如 `24h`, `7d`）。
...
- 使用 exec 批准 (`exec.approval.request`) 在调用 `system.run` 之前。
...
- `--agent <id>`: 代理范围内的批准/白名单（默认为配置的代理）。
