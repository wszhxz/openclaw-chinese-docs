...
- 未经身份验证绑定到回环之外的地址被阻止（安全防护措施）。
- 当授权时，`SIGUSR1` 触发进程内重启（默认启用 `commands.restart`；设置 `commands.restart: false` 以阻止手动重启，同时允许网关工具/配置应用/更新）。
...
- `--tailscale-reset-on-exit`: 关机时重置 Tailscale 提供/漏斗配置。
...
如果有多个网关可达，它会打印所有。当您使用隔离的配置文件/端口（例如，救援机器人）时，支持多个网关，但大多数安装仍然只运行一个网关。
...
- 多播 DNS-SD: `local.`
- 单播 DNS-SD（广域 Bonjour）：选择一个域名（示例：`openclaw.internal.`）并设置拆分 DNS + DNS 服务器；参见 [/gateway/bonjour](/gateway/bonjour)

只有启用了 Bonjour 发现（默认）的网关才会广播。
