...
本页面涵盖流式输出的调试辅助工具，特别是当提供程序将推理混入正常文本时。

## 运行时调试覆盖

在聊天中使用 `/debug` 来设置**仅运行时**配置覆盖（内存，非磁盘）。
默认情况下禁用 `/debug`；使用 `commands.debug: true` 启用。
当你需要切换模糊设置而无需编辑 `openclaw.json` 时，这非常方便。
...
使用开发配置文件来隔离状态并启动安全、可处置的调试设置。有**两个** `--dev` 标志：

- **全局 `--dev`（配置文件）：** 在 `~/.openclaw-dev` 下隔离状态并将网关端口默认设置为 `19001`（派生端口随其变化）。
...
2. **开发引导**（`gateway --dev`）
   ...
   - 默认身份：**C3‑PO**（协议机器人）。
   - 在开发模式下跳过频道提供程序（`OPENCLAW_SKIP_CHANNELS=1`）。
...
`--reset` 擦除配置、凭据、会话和开发工作区（使用 `trash`，不是 `rm`），然后重新创建默认开发设置。
...
> 注意：这只由使用pi-mono的进程发出
> `openai-completions` 提供程序。
