...
本页涵盖了流式输出的调试助手，特别是在提供者将推理混合到正常文本中时。

## 运行时调试覆盖

在聊天中使用 `/debug` 设置仅运行时的配置覆盖（内存中，而非磁盘）。
默认情况下禁用 `/debug`；使用 `commands.debug: true` 启用。
当你需要切换一些不常用的设置而无需编辑 `openclaw.json` 时，这非常方便。
...
使用开发配置文件来隔离状态，并为调试启动一个安全且可丢弃的设置。有两个 `--dev` 标志：
...
注意：`--dev` 是一个 **全局** 配置文件标志，可能会被某些运行器消耗。
如果需要明确指定，使用环境变量形式：
