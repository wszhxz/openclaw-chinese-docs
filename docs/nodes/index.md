...
一个 **node** 是一个伴生设备（macOS/iOS/Android/headless），它通过 `role: "node"` 连接到网关 **WebSocket**（与操作员使用相同的端口），并通过 `node.invoke` 暴露一个命令界面（例如 `canvas.*`, `camera.*`, `system.*`）。协议详情：[网关协议](/gateway/protocol)。

旧版传输：[桥接协议](/gateway/bridge-protocol)（TCP JSONL；已弃用/当前节点中已移除）。

macOS 也可以以 **节点模式** 运行：菜单栏应用程序连接到网关的 WS 服务器，并将其本地画布/摄像头命令作为节点暴露（因此 `openclaw nodes …` 可以针对此 Mac 工作）。
...
当您的网关运行在一个机器上而您希望命令在另一台机器上执行时，使用 **节点主机**。模型仍然与 **网关** 通信；当选择 `host=node` 时，网关
将 `exec` 调用转发到 **节点主机**。
...
- 令牌是从网关配置中的 `gateway.auth.token` (`~/.openclaw/openclaw.json` 在网关主机上)。
...
一旦设置，任何 `exec` 调用带有 `host=node` 都会在节点主机上运行（受节点允许列表/审批的约束）。
...
- 节点必须**置于前台**以使用`canvas.*`和`camera.*`（后台调用返回`NODE_BACKGROUND_UNAVAILABLE`）。
- 视频片段时长被限制（当前为`<= 60s`）以避免过大的base64负载。
- 当可能时，Android会提示请求`CAMERA`/`RECORD_AUDIO`权限；拒绝权限会导致`*_PERMISSION_REQUIRED`。
...
- `screen.record`需要节点应用程序置于前台。
- 在录制之前，Android会显示系统屏幕捕获提示。
- 屏幕录制被限制为`<= 60s`。
- `--no-audio`禁用麦克风捕获（在iOS/Android上支持；macOS使用系统捕获音频）。
- 使用`--screen <index>`在多个屏幕可用时选择一个显示器。

## 位置（节点）

当设置中启用位置服务时，节点暴露`location.get`。
...
- `system.run` 返回 stdout/stderr/exit code 在负载中。
...
- 在 macOS 节点模式下，`system.run` 受 macOS 应用中的执行审批限制（设置 → 执行审批）。
  请求/允许列表/完整行为与无头节点主机相同；被拒绝的提示返回 `SYSTEM_RUN_DENIED`。
- 在无头节点主机上，`system.run` 受执行审批限制 (`~/.openclaw/exec-approvals.json`)。

## 执行节点绑定

当有多个节点可用时，您可以将执行绑定到特定节点。
这设置了 `exec host=node` 的默认节点（每个代理可以覆盖）。
...
节点可能在 `permissions` 映射中包含 `node.list` / `node.describe`，按权限名称（例如 `screenRecording`, `accessibility`）键入布尔值 (`true` = 授予）。

## 无头节点主机（跨平台）

OpenClaw 可以运行一个 **无头节点主机**（无界面），它连接到网关
...
- 通过 `~/.openclaw/exec-approvals.json` 本地强制执行审批
  （参见 [执行审批](/tools/exec-approvals)）。
- 在 macOS 上，无头节点主机优先使用可访问的配套应用执行主机，如果应用不可用则回退到本地执行。设置 `OPENCLAW_NODE_EXEC_HOST=app` 以要求
