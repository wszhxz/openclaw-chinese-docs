...
OpenClaw 可以在回复管道运行之前 **总结入站媒体**（图像/音频/视频）。它会自动检测本地工具或提供商密钥是否可用，并且可以被禁用或自定义。如果理解功能关闭，模型仍然会像往常一样接收原始文件/URL。

## 目标

- 可选：预先将入站媒体摘要为短文本，以便更快地路由和更好的命令解析。
- 保留对模型的原始媒体传递（始终）。
- 支持 **提供商API** 和 **CLI 备用方案**。
- 允许多个模型按顺序备用（错误/大小/超时）。
...
- `{{OutputBase}}`（临时文件的基本路径，无扩展名）

## 默认值和限制

推荐的默认值：

- `maxChars`: **500** 对于图像/视频（简短，便于命令）
...
- `prompt` 默认为简单的“描述 {媒体}。”加上 `maxChars` 指南（仅限图像/视频）。
- 如果 `<capability>.enabled: true` 但没有配置模型，OpenClaw 尝试使用
  **活动回复模型** 当其提供商支持该功能时。

### 自动检测媒体理解（默认）

如果 `tools.media.<capability>.enabled` 不设置为 `false` 并且你没有
配置模型，OpenClaw 按此顺序自动检测并 **在第一个有效选项处停止**：
...
注意：二进制检测在 macOS/Linux/Windows 上尽力而为；确保 CLI 在 `PATH`（我们扩展 `~`），或者使用带有完整命令路径的显式 CLI 模型。
...
| 功能     | 提供商集成                                     | 备注                                              |
| -------- | ---------------------------------------------- | ------------------------------------------------- |
| 图像     | OpenAI / Anthropic / Google / 其他通过 `pi-ai` | 注册表中的任何图像功能模型都适用。              |
...
- `google/gemini-3-flash-preview`（快速），`google/gemini-3-pro-preview`（丰富）。
- CLI 备用方案：`gemini` CLI（支持视频/音频上的 `read_file`）。

## 附件策略

按功能 `attachments` 控制哪些附件被处理：

- `mode`: `first`（默认）或 `all`
- `maxAttachments`: 限制处理的数量（默认 **1**）
- `prefer`: `first`, `last`, `path`, `url`

当 `mode: "all"`，输出会被标记为 `[Image 1/2]`, `[Audio 2/2]` 等。
...
当媒体理解运行时，`/status` 包含简短的摘要行：

```
📎 Media: image ok (openai/gpt-5.2) · audio skipped (maxBytes)
```

这显示了按功能的结果以及适用的提供商/模型。
