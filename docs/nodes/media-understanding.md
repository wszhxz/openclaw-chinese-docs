...
- 可选：预先将入站媒体摘要为短文本以加快路由速度 + 提高命令解析质量。
- 保留对模型的原始媒体传递（始终）。
...
注意：二进制检测在 macOS/Linux/Windows 上尽力而为；确保 CLI 在 `PATH`（我们扩展 `~`），或者使用完整命令路径显式设置 CLI 模型。

## 功能（可选）

如果你设置了 `capabilities`，该条目仅针对这些媒体类型运行。对于共享列表，OpenClaw 可以推断默认值：
