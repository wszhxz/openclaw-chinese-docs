...
- 提供者身份验证遵循标准模型身份验证顺序（身份验证配置文件、环境变量、`models.providers.*.apiKey`）。
...
- 默认大小限制为 20MB (`tools.media.audio.maxBytes`)。超出大小的音频将被该模型跳过，并尝试下一个条目。
