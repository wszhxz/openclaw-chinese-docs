...
- 默认大小限制为 20MB (`tools.media.audio.maxBytes`)。超出大小的音频会被跳过该模型，并尝试下一个条目。
- 默认情况下，音频的 `maxChars` 是 **未设置**（完整转录）。设置 `tools.media.audio.maxChars` 或每个条目的 `maxChars` 来修剪输出。
...
- 范围规则使用首次匹配获胜。`chatType` 被标准化为 `direct`，`group`，或 `room`。
