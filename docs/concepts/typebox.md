...
TypeBox 是一个以 TypeScript 优先的模式库。我们使用它来定义 **Gateway
WebSocket 协议**（握手、请求/响应、服务器事件）。这些模式驱动 **运行时验证**、
**JSON 模式导出** 和 **Swift 代码生成** 用于 macOS 应用。一个真实来源；
其他一切都是生成的。

如果您想要更高级别的协议上下文，请从
[Gateway 架构](/concepts/architecture) 开始。
...
第一个帧 **必须** 是一个 `connect` 请求。之后，客户端可以调用
...
- **服务器端**：每个传入帧都用 AJV 验证。握手仅接受参数匹配 `ConnectParams` 的 `connect` 请求。
...
在 `src/gateway/server-methods.ts` 中注册（已经合并 `systemHandlers`），
...
- 具有 `req`、`res`、`event` 和 `unknown` 案例的 `GatewayFrame` 枚举
- 强类型有效载荷结构体/枚举
- `ErrorCode` 值和 `GATEWAY_PROTOCOL_VERSION`

未知帧类型作为原始有效载荷保留以实现向前兼容性。
...
- 大多数对象使用 `additionalProperties: false` 进行严格的有效载荷。
...
生成的 JSON 模式位于仓库中的 `dist/protocol.schema.json`。发布的原始文件通常可在以下位置获得：
