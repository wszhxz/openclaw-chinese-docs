...
- 密钥存放在 `~/.openclaw/agents/<agentId>/agent/auth-profiles.json` 中（旧版：`~/.openclaw/agent/auth-profiles.json`）。
...
配置文件存放在 `~/.openclaw/agents/<agentId>/agent/auth-profiles.json` 下的 `profiles` 中。
...
- **次键**：`usageStats.lastUsed`（每种类型中按最旧的排序）。
- **冷却/禁用的配置文件** 移动到最后，按最早过期的时间排序。

### 会话粘性（缓存友好）

OpenClaw **为每个会话固定选择的认证配置文件** 以保持提供商缓存热。
它不会在每个请求上轮换。固定的配置文件会被重用直到：

- 会话被重置 (`/new` / `/reset`)
- 压缩完成（压缩计数递增）
- 配置文件处于冷却/禁用状态

通过 `/model …@<profileId>` 手动选择会为该会话设置一个 **用户覆盖**，并且不会自动轮换，直到开始新会话。
...
如果你同时拥有同一提供商的 OAuth 配置文件和 API 密钥配置文件，循环会在消息之间在这两者之间切换，除非被固定。要强制使用单个配置文件：
...
当配置文件由于认证/速率限制错误（或看起来像速率限制的超时）而失败时，OpenClaw 会将其标记为冷却并移动到下一个配置文件。
...
账单/信用失败（例如“信用不足”/“信用余额太低”）被视为可故障转移的，但通常不是暂时性的。与其使用短暂的冷却，OpenClaw 会将配置文件标记为 **禁用**（带有更长的退避时间），并轮换到下一个配置文件/提供商。
...
当一个运行开始时带有模型覆盖（钩子或 CLI），即使尝试了任何配置的回退，回退也会在 `agents.defaults.model.primary` 结束。
