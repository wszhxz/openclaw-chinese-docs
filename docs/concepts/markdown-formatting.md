...
OpenClaw 通过将其转换为共享中间表示（IR）来格式化出站 Markdown，然后再渲染特定于通道的输出。IR 保留了源文本，同时携带样式/链接跨度，以便在通道之间保持分块和渲染的一致性。
...
1. **解析 Markdown -> IR**
   ...
   - 偏移量是 UTF-16 代码单元，因此 Signal 样式范围与它的 API 对齐。
   - 表格仅在通道选择进行表格转换时才被解析。
2. **分块 IR（格式优先）**
   - 分块发生在渲染之前的 IR 文本上。
   - 内联格式不会跨块拆分；跨度按块切片。
...
- 其他通道（WhatsApp、iMessage、MS Teams、Discord）仍然使用纯文本或它们自己的格式规则，并在启用时在分块之前应用 Markdown 表格转换。

## 表格处理

Markdown 表格在聊天客户端中并不一致地支持。使用 `markdown.tables` 按通道（和账户）控制转换。
...
- 内联样式（粗体/斜体/删除线/内联代码/spoiler）永远不会跨块拆分；渲染器在每个块中重新打开样式。

如果需要了解跨通道的分块行为，请参阅 [流式传输 + 分块](/concepts/streaming)。

## 链接策略

- **Slack：** `[label](url)` -> `<url|label>`；裸 URL 保持不变。解析期间禁用自动链接以避免双链接。
