...
OpenClaw 通过将其转换为共享中间表示（IR）来格式化出站 Markdown，然后再渲染特定于通道的输出。IR 保留源文本的同时携带样式/链接范围，以便在各个通道中分块和渲染保持一致。
...
1. **解析 Markdown -> IR**
   ...
   - 偏移量是 UTF-16 代码单元，以便 Signal 样式范围与 API 对齐。
   - 表格仅在通道选择进行表格转换时解析。
2. **分块 IR（格式优先）**
   - 分块发生在渲染之前的 IR 文本上。
   - 内联格式不会跨块拆分；范围按块切片。
...
- 其他通道（WhatsApp、iMessage、MS Teams、Discord）仍然使用纯文本或其自己的格式规则，在启用时会在分块前应用 Markdown 表格转换。

## 表格处理

Markdown 表格在聊天客户端中支持不一致。使用 `markdown.tables` 按通道（和账户）控制转换。
...
- 代码块被保留为一个带有尾随换行符的单个块，以便通道正确渲染它们。
- 列表前缀和引用前缀是 IR 文本的一部分，因此分块不会在前缀中途拆分。
- 内联样式（粗体/斜体/删除线/内联代码/剧透）永远不会跨块拆分；渲染器在每个块中重新打开样式。

如果需要更多关于通道间分块行为的信息，请参见 [流式传输 + 分块](/concepts/streaming)。
...
剧透标记 (`||spoiler||`) 仅解析为 Signal，其中它们映射到 SPOILER 样式范围。其他通道将其视为纯文本。
