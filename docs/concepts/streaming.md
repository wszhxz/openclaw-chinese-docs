...
- **类似令牌的流式传输（仅限 Telegram）：** 在生成时使用部分文本更新**草稿气泡**；最终消息在最后发送。
...
- 通道分块模式：`*.chunkMode`（默认为 `length`，`newline` 在长度分块之前按空白行（段落边界）拆分）。
- Discord 软限制：`channels.discord.maxLinesPerMessage`（默认 17）拆分高大的回复以避免 UI 裁剪。
...
`message_end` 仍然会在缓冲文本超过 `maxChars` 时使用分块器，因此可以在末尾发出多个块。
...
- **代码围栏：** 永不在围栏内拆分；当在 `maxChars` 被强制时，关闭并重新打开围栏以保持 Markdown 的有效性。

`maxChars` 被限制在通道 `textChunkLimit` 内，因此不能超过每个通道的限制。

## 合并（合并流式传输的块）

启用块流式传输时，OpenClaw 可以在发送之前**合并连续的块块**。这减少了“单行垃圾信息”，同时仍提供渐进式输出。

- 合并等待**空闲间隔**(`idleMs`) 之前刷新。
- 缓冲区被 `maxChars` 限制，如果超出则会刷新。
- `minChars` 防止小片段在积累足够的文本之前发送（最终刷新总是发送剩余的文本）。
...
启用块流式传输时，你可以在**块回复之间**添加一个**随机暂停**（在第一个块之后）。这使多气泡响应感觉更自然。
...
- **流式传输块：** `blockStreamingDefault: "on"` + `blockStreamingBreak: "text_end"`（实时发出）。非 Telegram 通道还需要 `*.blockStreaming: true`。
...
- 草稿流式传输与块流式传输分开；块回复默认关闭，并且仅通过 `*.blockStreaming: true` 在非 Telegram 通道上启用。
- 最终回复仍然是正常消息。
