...
目标: Clawd 风格的工作区 (`agents.defaults.workspace`, 默认 `~/.openclaw/workspace`)，其中“内存”以每天一个 Markdown 文件的形式存储 (`memory/YYYY-MM-DD.md`) 加上一组小的稳定文件（例如 `memory.md`, `SOUL.md`）。
...
保持 `~/.openclaw/workspace` 作为权威的人类可读内存。
...
- `memory.md` 保持“小 + 核心”：你希望 Clawd 在每次会话中看到的东西。
...
- 用于事实 + 实体链接 + 意见元数据的 SQLite 模式
- 用于词法回忆的 SQLite **FTS5**（快速、小巧、离线）
- 可选的嵌入表用于语义回忆（仍然离线）
...
Hindsight 在这里的关键见解：存储**叙述性的、自包含的事实**，而不是小片段。
...
如果你不想让作者考虑这些：反射作业可以从日志的其余部分推断这些要点，但有一个显式的 `## Retain` 部分是最简单的“质量杠杆”。
...
- 每个意见都有：
  ...
  - 证据链接（支持 + 反对的事实 ID）
- 当新事实到达时：
  - 通过实体重叠 + 相似性找到候选意见（首先使用 FTS，然后使用嵌入）
...
- HNSW 索引（常见，稳健；需要库绑定）
- SuCo（研究级；如果有可靠的实现可以嵌入，则具有吸引力）

开放问题：

- 你机器（笔记本电脑 + 桌面）上“个人助理内存”的**最佳**离线嵌入模型是什么？
  - 如果你已经有 Ollama：使用本地模型进行嵌入；否则在工具链中附带一个小的嵌入模型。
...
- Letta / MemGPT 概念：“核心记忆块” + “档案记忆” + 工具驱动的自我编辑记忆。
