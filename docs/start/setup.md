...
- **稳定的工作流:** 安装macOS应用程序；让它运行捆绑的网关。
- **前沿的工作流:** 通过`pnpm gateway:watch`自行运行网关，然后让macOS应用程序以本地模式附加。
...
如果您希望“完全针对我”_并且_易于更新，请将自定义内容保留在：
...
如果您还没有全局安装，请通过`pnpm openclaw setup`运行它。

## 稳定的工作流（先安装macOS应用程序）

1. 安装并启动 **OpenClaw.app**（菜单栏）。
2. 完成入站检查/权限清单（TCC提示）。
3. 确保网关是**本地**且正在运行（应用程序会管理它）。
4. 链接表面（示例：WhatsApp）：
...
目标：在TypeScript网关上工作，获取热重载，并保持macOS应用程序UI附加。
...
- **错误的端口:** 网关WS默认为 `ws://127.0.0.1:18789`；保持应用程序和CLI在同一端口。
- **状态存储位置:**
...
Linux安装使用systemd **用户**服务。默认情况下，systemd会在注销/空闲时停止用户服务，这会终止网关。入站尝试为您启用持久性（可能会提示输入sudo）。如果仍然关闭，请运行：

```bash
sudo loginctl enable-linger $USER
```

对于始终在线或多用户服务器，考虑使用**系统**服务而不是用户服务（不需要持久性）。有关systemd说明，请参阅[网关运行手册](/gateway)。
