...
- 代理是唯一的slug（例如 `main`，`research`）。网关会暴露列表。
- 会话属于当前代理。
- 会话密钥存储为 `agent:<agentId>:<sessionKey>`。
  - 如果您输入 `/session main`，TUI会将其展开为 `agent:<currentAgent>:main`。
...
- 在行首加上 `!` 以在TUI主机上运行本地shell命令。
- TUI在每个会话中提示一次以允许本地执行；拒绝将使 `!` 在该会话中保持禁用。
- 命令在一个新的、非交互式的shell中运行，位于TUI的工作目录中（没有持久化的 `cd`/env）。
