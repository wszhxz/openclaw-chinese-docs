...
5. 在 [Google Cloud 控制台 Chat 配置](https://console.cloud.google.com/apis/api/chat.googleapis.com/hangouts-chat) 中创建一个 Google Chat 应用：
   ...
   - 在 **触发器** 下，选择 **对所有触发器使用通用 HTTP 终端点 URL** 并设置为你的网关的公共 URL 加上 `/googlechat`。
...
3. 在搜索栏（通常用于添加人员的地方），输入你在 Google Cloud 控制台中配置的 **应用名称**。
   - **注意**：机器人不会出现在“市场”浏览列表中，因为它是一个私有应用。必须按名称搜索它。
4. 从结果中选择你的机器人。
...
Google Chat 网络钩子需要一个公共 HTTPS 终端点。出于安全考虑，**仅将 `/googlechat` 路径** 暴露到互联网。将 OpenClaw 仪表板和其他敏感终端点保留在私有网络中。

### 选项 A：Tailscale Funnel（推荐）

使用 Tailscale Serve 用于私有仪表板，Funnel 用于公共网络钩子路径。这使 `/` 保持私有，同时仅暴露 `/googlechat`。
