...
状态：通过 Google Chat API Webhook（仅限HTTP）已准备好用于直接消息和空间。

## 快速设置（初学者）

1. 创建一个 Google Cloud 项目并启用 **Google Chat API**。
   - 访问：[Google Chat API 凭证](https://console.cloud.google.com/apis/api/chat.googleapis.com/credentials)
   - 如果尚未启用，请启用该API。
2. 创建一个 **服务账号**：
   - 点击 **创建凭据** > **服务账号**。
...
5. 在 [Google Cloud 控制台 Chat 配置](https://console.cloud.google.com/apis/api/chat.googleapis.com/hangouts-chat) 中创建一个 Google Chat 应用：
   ...
   - 在 **触发器** 下，选择 **为所有触发器使用通用 HTTP 终端点 URL**，并将其设置为你的网关的公共 URL 加上 `/googlechat`。
...
3. 在搜索栏（通常用于添加人员的地方），输入你在 Google Cloud 控制台中配置的 **应用名称**。
   - **注意**：机器人不会出现在“市场”浏览列表中，因为它是一个私有应用。你必须按名称搜索它。
4. 从结果中选择你的机器人。
...
Google Chat Webhook 需要一个公共 HTTPS 终端点。出于安全考虑，**仅将 `/googlechat` 路径**暴露给互联网。将 OpenClaw 仪表板和其他敏感终端点保留在私有网络上。

### 选项 A: Tailscale Funnel（推荐）

使用 Tailscale Serve 用于私有仪表板，Funnel 用于公共 Webhook 路径。这将使 `/` 保持私有，同时仅暴露 `/googlechat`。
...
> 注意：此配置在重启后仍然有效。稍后要删除它，请运行 `tailscale funnel reset` 和 `tailscale serve reset`。
...
4. 默认情况下，直接消息访问需要配对。未知发送者将收到配对代码；通过以下方式批准：
...
- 附件通过 Chat API 下载并通过媒体管道存储（大小由 `mediaMaxMb` 限制）。
...
1. **通道未配置**：你的配置中缺少 `channels.googlechat` 部分。验证：
   ...
   如果返回 "Config path not found"，添加配置（参见 [配置亮点](#config-highlights)）。
