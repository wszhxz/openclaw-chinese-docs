...
当您需要 OpenClaw 在经典频道 (`#room`) 和直接消息中工作时，请使用 IRC。
...
……这意味着发送者未被允许发送 **群组/频道** 消息。通过以下方式修复：
...
即使频道已允许（通过 `groupPolicy` + `groups`）且发送者已允许，OpenClaw 在群组上下文中默认采用 **提及限制**。

这意味着除非消息包含匹配机器人的提及模式，否则您可能看到类似 `drop channel … (missing-mention)` 的日志。

要使机器人在 IRC 频道中 **无需提及即可回复**，请禁用该频道的提及限制：
...
或者要允许 **所有** IRC 频道（无每频道白名单）并且仍然无需提及即可回复：
...
- 旧版无前缀键仍被接受，但仅作为 `id:` 进行匹配。
- 第一个匹配的发送者策略生效；`"*"` 是通配符回退。

有关群组访问与提及限制的更多信息（以及它们如何交互），请参阅：[/channels/groups](/channels/groups)。
...
- 如果机器人连接但从未在频道中回复，请验证 `channels.irc.groups` **以及** 提及限制是否丢弃了消息 (`missing-mention`)。如果您希望它在无需 ping 的情况下回复，请为该频道设置 `requireMention:false`。
