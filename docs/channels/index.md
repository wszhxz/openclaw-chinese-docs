...
OpenClaw 可以通过您已经使用的任何聊天应用程序与您交流。每个频道通过网关连接。
...
- [BlueBubbles](/channels/bluebubbles) — **推荐用于 iMessage**；使用 BlueBubbles macOS 服务器 REST API，支持全部功能（编辑、撤回、效果、反应、群组管理 — 目前在 macOS 26 Tahoe 上编辑功能失效）。
...
- 最快的设置通常是 **Telegram**（简单的机器人令牌）。WhatsApp 需要 QR 配对，并且会在磁盘上存储更多状态。
- 群组行为因频道而异；参见 [群组](/channels/groups)。
- 强制执行私信配对和允许列表以确保安全；参见 [安全性](/gateway/security)。
