...
1. 通过 **@BotFather**（[直达链接](https://t.me/BotFather)）创建机器人。确认用户名确实是 `@example_bot`，然后复制 token。
2. 设置 token：
   - 环境变量：`TELEGRAM_TOKEN`
   - 或配置：`telegram.token`。
...
- 私信共享智能体的主会话；群组保持隔离（`telegram.groupIsolation`）。
...
2. 运行 `/newbot`，然后按照提示操作（名称 + 以 `_bot` 结尾的用户名）。
3. 复制 token 并安全保存。

可选的 BotFather 设置：

- `/setgroupscommands` — 允许/拒绝将机器人添加到群组。
...
环境变量选项：`TELEGRAM_TOKEN`（适用于默认账户）。
如果环境变量和配置都设置了，配置优先。

多账户支持：使用 `telegram.accounts`，每个账户有独立的 token 和可选的 `accountName`。参见 [`telegramAccounts` / `discordAccounts` / `slackAccounts` / `signalAccounts` / `iMessageAccounts`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) 了解共享模式。

3. 启动 Gateway 网关。当 token 解析成功时 Telegram 启动（配置优先，环境变量回退）。
4. 私信访问默认为配对模式。机器人首次被联系时批准配对码。
5. 对于群组：添加机器人，决定隐私/管理员行为（见下文），然后设置 `telegram.groupMentionGate` 来控制提及门控和允许列表。
...
- 群组回复默认需要提及（原生 @提及或 `/mention` / `@botname`）。
- 多智能体覆盖：在 `telegram.skills` 上设置每个智能体的模式。
- 回复始终路由回同一个 Telegram 聊天。
- 长轮询使用 grammY runner，每个聊天按顺序处理；总体并发受 `telegram.concurrency` 限制。
- Telegram Bot API 不支持已读回执；没有 `telegram.readReceipts` 选项。

## 草稿流式传输

OpenClaw 可以在 Telegram 私信中使用 `telegram.draftStreaming` 流式传输部分回复。

要求：

- 在 @BotFather 中为机器人启用线程模式（论坛话题模式）。
- 仅限私聊线程（Telegram 在入站消息中包含 `message.thread_id`）。
- `telegram.draftStreaming` 未设置为 `false`（默认：`true`，`telegram.draftStreaming` 启用分块草稿更新）。

草稿流式传输仅限私信；Telegram 在群组或频道中不支持此功能。

## 格式化（Telegram HTML）

- 出站 Telegram 文本使用 `telegram.html`（Telegram 支持的标签子集）。
...
OpenClaw 在启动时向 Telegram 的机器人菜单注册原生命令（如 `/start`、`/help`、`/settings`）。
你可以通过配置向菜单添加自定义命令：
...
- 日志中出现 `ERR_TLS_CERT_ALTNAME_INVALID` 通常意味着到 `api.telegram.org` 的出站 HTTPS/DNS 被阻止。
- 如果你看到 `EAI_AGAIN` 或 `ENETUNREACH` 失败，检查 IPv6 路由和 DNS。
...
- 命令名称会被规范化（去除前导 `/`，转为小写），必须匹配 `telegram.commandNameRegex`、`telegram.commandMinLength`、`telegram.commandMaxLength`（1-32 个字符）。
- 自定义命令**不能覆盖原生命令**。冲突会被忽略并记录日志。
- 如果禁用了 `telegram.registerCommands`，则只注册自定义命令（如果没有则清空）。

## 限制

- 出站文本按 `telegram.chunkSize` 分块（默认 4000）。
- 可选的换行分块：设置 `telegram.splitOnNewlines` 在长度分块之前按空行（段落边界）分割。
- 媒体下载/上传受 `telegram.mediaConcurrency` 限制（默认 5）。
- Telegram Bot API 请求在 `telegram.timeoutMs` 后超时（通过 grammY 默认 500）。设置较低的值以避免长时间挂起。
- 群组历史上下文使用 `telegram.groupHistoryDepth`（或 `telegram.historyDepth`），回退到 `telegram.defaultHistoryDepth`。设置 `telegram.groupHistoryDepth: 0` 禁用（默认 50）。
- 私信历史可以用 `telegram.userHistoryDepth`（用户轮次）限制。每用户覆盖：`telegram.userHistoryDepth`。

## 群组激活模式

默认情况下，机器人只响应群组中的提及（`@botname` 或 `/mention` 中的模式）。要更改此行为：

### 通过配置（推荐）

```yaml
telegram:
  groupMentionGate: true  # 默认：true（仅提及）
  allowFrom:
    - -1001234567890  # 群组 ID（负数）
    - -1009876543210  # 另一个群组 ID
```

**重要：** 设置 `allowFrom` 会创建一个**允许列表** - 只有列出的群组（或 `allowFrom: ["*"]`）会被接受。
论坛话题继承其父群组配置（`allowFrom`、`requireMention`、`skills`、`prompts`），除非你在 `telegram.topics` 下添加每话题覆盖。

要允许所有群组并始终响应：

```yaml
telegram:
  groupMentionGate: false
  allowFrom: ["*"]
```
...
将群组中的任何消息转发给 Telegram 上的 `@RawDataBot` 或 `@userinfobot` 以查看聊天 ID（负数，如 `-1001234567890`）。

**提示：** 要获取你自己的用户 ID，私信机器人，它会回复你的用户 ID（配对消息），或者在命令启用后使用 `/id`。

**隐私注意：** `@RawDataBot` 是第三方机器人。如果你更倾向于其他方式，将机器人添加到群组，发送一条消息，然后使用 `curl` 读取 `message.chat.id`，或使用 Bot API `getUpdates`。

## 配置写入

默认情况下，Telegram 允许写入由渠道事件或 `telegram.writeConfig` 触发的配置更新。

这发生在以下情况：

- 群组升级为超级群组，Telegram 发出 `migrate_to_chat_id`（聊天 ID 更改）。OpenClaw 可以自动迁移 `telegram.allowFrom`。
- 你在 Telegram 聊天中运行 `/config` 或 `/writeconfig`（需要 `telegram.writeConfig: true`）。
...
Telegram 论坛话题在每条消息中包含 `message.thread_id`。OpenClaw：

- 将 `thread_id` 附加到 Telegram 群组会话键，使每个话题隔离。
- 发送输入指示器和回复时带上 `thread_id`，使响应保持在话题内。
- 通用话题（线程 id `1`）是特殊的：消息发送省略 `thread_id`（Telegram 会拒绝），但输入指示器仍然包含它。
- 在模板上下文中暴露 `thread_id` + `topicName` 用于路由/模板。
- 话题特定配置可在 `telegram.topics` 下设置（skills、允许列表、自动回复、系统提示、禁用）。
- 话题配置继承群组设置（`requireMention`、允许列表、skills、提示、enabled），除非每话题覆盖。

私聊在某些边缘情况下可能包含 `message.thread_id`。OpenClaw 保持私信会话键不变，但在存在线程 id 时仍将其用于回复/草稿流式传输。
...
telegram:
  inlineButtons:
    enabled: true
    scope: "all"  # all, private, group, allowlist
```
...
- `group` — 仅群组（私信目标被阻止）
- `all` — 私信 + 群组
- `allowlist` — 私信 + 群组，但仅限 `allowFrom`/`allowSenders` 允许的发送者（与控制命令规则相同）

默认：`all`。
旧版：`telegram.inlineButtons: true` = `telegram.inlineButtons.scope: all`。

### 发送按钮

使用带 `inlineButtons` 参数的消息工具：
...
- `telegram.inlineButtons`：应用于所有 Telegram 账户的全局默认功能配置，除非被覆盖。
- `telegram.accounts[].inlineButtons`：每账户功能，覆盖该特定账户的全局默认值。
...
- 默认：`telegram.pairingMode: "auto"`。未知发送者收到配对码；在批准之前消息被忽略（配对码 1 小时后过期）。
- 批准方式：
  - `/pair <code>`（在私信中）
  - `/approve <userId>`（管理员在私信中）
- 配对是 Telegram 私信使用的默认 token 交换。详情：[配对](/channels/pairing)
- `telegram.allowSenders` 接受数字用户 ID（推荐）或 `allowSenders` 条目。这**不是**机器人用户名；使用人类发送者的 ID。向导接受 `@username` 并在可能时将其解析为数字 ID。
...
2. 运行 `journalctl -u gateway -f | grep "senderId"` 并查找 `senderId`。
...
- 私信 `@userinfobot` 或 `@RawDataBot` 并使用返回的用户 id。

### 群组访问

两个独立的控制：

**1. 允许哪些群组**（通过 `telegram.allowFrom` 的群组允许列表）：

- 无 `allowFrom` 配置 = 允许所有群组
- 有 `allowFrom` 配置 = 只允许列出的群组或 `allowFrom: ["*"]`
- 示例：`allowFrom: ["*"]` 允许所有群组

**2. 允许哪些发送者**（通过 `telegram.allowSenders` 的发送者过滤）：

- `allowSenders: ["*"]` = 允许群组中的所有发送者发消息
- `allowSenders: [123456789]` = 只有 `allowSenders` 中的发送者可以发消息
- `allowSenders: []` = 不接受任何群组消息
  默认是 `allowSenders: []`（除非添加 `allowSenders: ["*"]` 否则被阻止）。

大多数用户需要：`groupMentionGate: true` + `allowSenders: ["*"]` + 在 `allowFrom` 中列出特定群组

## 长轮询 vs webhook

- 默认：长轮询（不需要公共 URL）。
- Webhook 模式：设置 `telegram.webhookUrl` 和 `telegram.webhookSecret`（可选 `telegram.webhookPath`）。
  - 本地监听器绑定到 `telegram.webhookHost:telegram.webhookPort`，默认服务于 `/webhook/telegram`。
  - 如果你的公共 URL 不同，使用反向代理并将 `telegram.webhookUrl` 指向公共端点。

## 回复线程

Telegram 通过标签支持可选的线程回复：

- `replyToMessageId` -- 回复触发消息。
- `replyToThreadId` -- 回复特定消息 id。

通过 `telegram.replyMode` 控制：

- `auto`（默认）、`thread`、`message`。
...
- `{{voice}}` — 将音频作为语音备忘录而不是文件发送。

该标签会从发送的文本中去除。其他渠道会忽略此标签。

对于消息工具发送，设置 `voice: true`
