...
身份验证是按智能体进行的：每个智能体从自己的 `agentDir` 身份验证存储中读取：

```
~/.openclaw/agents/<agentId>/agent/auth-profiles.json
```

智能体之间不共享凭据。不要在智能体之间重用 `agentDir`。
如果您想共享凭据，请将 `auth-profiles.json` 复制到其他智能体的 `agentDir` 中。

有关沙箱在运行时的行为，请参阅 [沙箱](/gateway/sandboxing)。
有关调试“为什么被阻止了？”请参阅 [沙箱 vs 工具策略 vs 提升](/gateway/sandbox-vs-tool-policy-vs-elevated) 和 `openclaw sandbox explain`。
...
- `agents.list[].sandbox.{docker,browser,prune}.*` 对该智能体覆盖 `agents.defaults.sandbox.{docker,browser,prune}.*`（当沙箱范围解析为 `"shared"` 时忽略）。
...
`tools.elevated` 是全局基线（基于发送者的白名单）。`agents.list[].tools.elevated` 可以进一步限制特定智能体的提升（两者都必须允许）。

缓解模式：

- 拒绝不受信任智能体的 `exec` (`agents.list[].tools.deny: ["exec"]`)
- 避免将路由到受限智能体的发送者列入白名单
- 如果只想进行沙箱化执行，请全局禁用提升 (`tools.elevated.enabled: false`)
- 对敏感配置文件禁用提升 (`agents.list[].tools.elevated.enabled: false`)
...
旧的 `agent.*` 配置通过 `openclaw doctor` 迁移；以后建议使用 `agents.defaults` + `agents.list`。
...
而不是智能体 ID。群组/频道会话始终有自己的密钥，因此被视为非主，并且会被沙箱化。如果您希望某个智能体永不被沙箱化，请设置 `agents.list[].sandbox.mode: "off"`。
