...
有关调试“为什么被阻止？”的问题，请参阅 [沙箱与工具策略与提升](/gateway/sandbox-vs-tool-policy-vs-elevated) 和 `openclaw sandbox explain`。
...
6. **特定智能体提供者策略** (`agents.list[].tools.byProvider[provider].allow/deny`)
...
- `group:nodes`: `nodes`
- `group:openclaw`: 所有内置的 OpenClaw 工具（排除提供者插件）
...
而不是智能体 ID。组/频道会话始终获得自己的密钥，因此被视为“非主”，并将被沙箱化。如果你希望某个智能体永远不被沙箱化，请设置 `agents.list[].sandbox.mode: "off"`。
