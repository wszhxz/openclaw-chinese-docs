...
要了解实际分解（按注入文件、工具、技能和系统提示大小），使用 `/context list` 或 `/context detail`。参见 [上下文](/concepts/context)。

## 如何查看当前令牌使用情况

在聊天中使用这些命令：

- `/status` → 带有会话模型、上下文使用情况、上次响应输入/输出令牌的**表情丰富状态卡**，以及**预估成本**（仅限 API 密钥）。
...
选择性地运行**缓存-TTL 修剪**：缓存 TTL
过期后修剪会话，然后重置缓存窗口，以便后续请求可以重用
刚缓存的上下文，而不是重新缓存完整历史记录。这在会话超过 TTL 闲置时
降低缓存写入成本。

在 [网关配置](/gateway/configuration) 中配置它，并在
...
对于 Anthropic API 定价，缓存读取比输入令牌便宜得多，
而缓存写入则按更高的倍数计费。请参阅 Anthropic 的
提示缓存定价了解最新费率和 TTL 倍数：
