...
- `/status` 显示当前会话模型、上下文使用情况以及最后回复的标记数。
- 如果模型使用**API密钥认证**，`/status` 还会显示最后回复的**估算费用**。

**每消息费用页脚**

- `/usage full` 为每个回复附加一个使用情况页脚，包括**估算费用**（仅限API密钥）。
- `/usage tokens` 仅显示标记数；OAuth流程隐藏美元费用。
...
- **技能** (`skills.entries.<name>.apiKey`)，这些技能可能将密钥导出到技能进程环境。
...
传入的媒体可以在回复运行之前进行摘要/转录。这使用了模型/提供商API。
...
- `memorySearch.provider = "openai"` → OpenAI嵌入
- `memorySearch.provider = "gemini"` → Gemini嵌入
- 如果本地嵌入失败，可选回退到OpenAI

您可以将其保留在本地使用 `memorySearch.provider = "local"`（不使用API）。
...
`web_fetch` 当存在API密钥时可以调用**Firecrawl**：
...
某些状态命令调用**提供商使用端点**以显示配额窗口或认证健康状况。
