...
OpenClaw 使用一个专用的工作区目录来存放代理文件。默认值: `~/.openclaw/workspace` （可通过 `agents.defaults.workspace` 进行配置）。
...
- 默认情况下，直接聊天会合并到代理的 `main` 会话中；群组保持隔离作为 `agent:<agentId>:<channel>:group:<id>`（房间/频道：`agent:<agentId>:<channel>:channel:<id>`）；心跳保持后台任务活跃。
...
- **oracle** — 准备好与 OpenAI 配合使用的代理 CLI，带有会话重播和浏览器控制。
- **eightctl** — 从终端控制睡眠。
...
- **OpenAI Whisper** — 本地语音转文本进行快速速记和语音邮件转录。
