...
- [ ] 确认包元数据（名称、描述、存储库、关键字、许可证）和 `bin` 映射指向 [`openclaw.mjs`](https://github.com/openclaw/openclaw/blob/main/openclaw.mjs) 对于 `openclaw`。
- [ ] 如果依赖项更改，运行 `pnpm install` 使 `pnpm-lock.yaml` 最新。
...
- [ ] （可选）完整安装程序冒烟（添加非 root + CLI 覆盖）：`pnpm test:install:smoke`
- [ ] （可选）安装程序端到端（Docker，运行 `curl -fsSL https://openclaw.ai/install.sh | bash`，注册，然后运行实际工具调用）：
...
- [ ] 保留应用压缩包（和可选的 dSYM 压缩包）以便附加到 GitHub 发布。
...
- **npm pack/publish 悬挂或生成巨大的 tarball**：macOS 应用包在 `dist/OpenClaw.app`（和发布压缩包）中被扫入包中。通过 `package.json` `files` 白名单发布内容（包括 dist 子目录、docs、skills；排除应用包）修复。使用 `npm pack --dry-run` 确认 `dist/OpenClaw.app` 未列出。
- **npm auth web 循环用于 dist-tags**：使用旧版身份验证以获取 OTP 提示：
  - `NPM_CONFIG_AUTH_TYPE=legacy npm dist-tag add openclaw@X.Y.Z latest`
- **`npx` 验证失败，错误 `ECOMPROMISED: Lock compromised`**：使用新鲜缓存重试：
...
我们仅在 `@openclaw/*` 范围下发布**现有 npm 插件**。未在 npm 上的捆绑插件保持**仅磁盘树**（仍包含在 `extensions/**` 中）。
